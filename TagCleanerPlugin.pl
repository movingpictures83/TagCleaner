...
#perl tagcleaner.pl -fastq /biorg/data/RNA/UMiami/small/S12_R1.fastq -tag3 TGTACCCACGCC -out myoutput
sub input {
   open(FH, '<', $_[0])
...
sub output {
	$params{fastq} = $file_data{fastq};
	$params{tag3} = $file_data{tag3};
	$params{out} = $_[0];
my ($file1,$command);
...
#check tag sequences
$MAX_TAG_LEN = 32; #depending on the system, use up to 64 for 64 bit systems
...
if($filenamegood2) {
    &printLog("Write quality results to file: \"".$filenamegood2."\"");
}
...
$ALPHABET    = 'ACGTN';
$ALPHABET_IUPAC = $ALPHABET.'RYMKWSBDHV';
...
my @ns;
(my $ntmp = uc($TAG5)) =~ s/^(N*)[^N]*(N*)$/$1$2/;
push(@ns,length($ntmp));
($ntmp = uc($TAG3)) =~ s/^(N*)[^N]*(N*)$/$1$2/;
push(@ns,length($ntmp));
    ...
    while(<FILE>) {
        ...
        if($count == 0 && /^\@(\S+)\s*(.*)$/) {
            ...
            if($seq && $length) {
                ...
                $numbases += $length;
                if($PREDICT) {
                    &getKmersAndBaseFreqs(\%kmers,\%basefreqs,uc($seq),$length,$KMER_LEN,&max($MAX_WEB_LEN,$MAX_TAG_LEN));
                } else {
                    #identify tag
		    #print($m5, $m3, $Peq5, $Peq3, $Peq5rev, $Peq3rev, $Pv05, $Pv03);
                    my ($trims,$minscores,$splitstatus) = &searchForTag([$m5,$m3],[$Peq5,$Peq3,$Peq5rev,$Peq3rev],[$Pv05,$Pv03],[$initlen5,$initlen3],[0,0],[$minmove5,$minmove3],uc($seq),$length,\@sp,\@ns);
                    #process sequence
                    if($STATS) {
                        #calc summary stats
        ...
        if(/^>(\S+)\s*(.*)$/) {
            ...
            if($seq && $length) {
                ...
                if($PREDICT) {
                    ...
                    #identify tag
                    my ($trims,$minscores,$splitstatus) = &searchForTag([$m5,$m3],[$Peq5,$Peq3,$Peq5rev,$Peq3rev],[$Pv05,$Pv03],[$initlen5,$initlen3],[$MM5,$MM3],[$minmove5,$minmove3],uc($seq),$length,\@sp,\@ns);
                    #process sequence
                    if(exists $params{stats}) {
                        #calc summary stats
                        foreach my $k (keys %$splitstatus) {
...
if(exists $params{verbose} && !$STATS && !$PREDICT) {
    { no integer;
        ...
        print STDERR "\tInput mean length: ".sprintf("%.2f",($numseqs > 0 ? $numbases/$numseqs : 0))."\n";
        print STDERR "\tOutput sequences: ".&addCommas($numseqsout)."\n";
        print STDERR "\tOutput bases: ".&addCommas($numbasesout)."\n";
        print STDERR "\tOutput mean length: ".sprintf("%.2f",($numseqsout > 0 ? $numbasesout/$numseqsout : 0))."\n";
        &printLog("Input sequences: ".&addCommas($numseqs));
        &printLog("Input bases: ".&addCommas($numbases));
        &printLog("Input mean length: ".sprintf("%.2f",($numseqs > 0 ? $numbases/$numseqs : 0)));
        &printLog("Output sequences: ".&addCommas($numseqsout));
        &printLog("Output bases: ".&addCommas($numbasesout));
        &printLog("Output mean length: ".sprintf("%.2f",($numseqsout > 0 ? $numbasesout/$numseqsout : 0)));
    }
}
...
if($PREDICT) {
    ...
    my ($kmersum,$kmershift) = &filterAndShiftKmers(\%kmers, $numseqs);
...
sub getPeq {
	...
	foreach my $base (@rev_seq) {
            $Peq{$char} <<= 1;
            if(exists $MATRIX->{uc($base)}->{$char}) { #include wild cards (iupac chars)
                $Peq{$char} |= 1;
...
#                        $used{5}->{$i} = 1;
    ...
    #get [start,length] tuple from end and internal tags
...
sub filterAndShiftKmers {
	...
	if($numkmer > 1) {
		...
		foreach my $i (0..($countgood-1)) {
		    $kmershift{$sp}->{$kmersort[$i+1]} = (defined $matrix[0]->[$i]->[0] ? $matrix[0]->[$i]->[0] : 0)+$min;
...
sub predictTag {
    ...
    $count = 0;
    foreach my $i (@index) {
	if(($i == 0 || $ranges[$i] > $medians[$i]*3 || ($medians[$i] == 0 && $ranges[$i] > 0)) && $tmp ne 'E' && $tmp ne 'N' && scalar(@bases) && defined $bases[0]) {
	    $tag .= shift(@bases);
...
sub processData {
    ...
    if(($NOMATCH == 5 && $num > 1) ||
       ...
       ($NOMATCH == 2 && $num > 0 && ($trims->[$num-1]->[0]+$trims->[$num-1]->[1] == $length)) ||
       ($NOMATCH == 3 && $num > 0 && ($trims->[0]->[0] == 0 || ($trims->[$num-1]->[0]+$trims->[$num-1]->[1] == $length))) ||
       ($NOMATCH == 4 && $num > 0 && $trims->[0]->[0] == 0 && ($trims->[$num-1]->[0]+$trims->[$num-1]->[1] == $length))) {
        $filter = 1;
    }
